...
function displayRoleList($dataRole)
{
    ...
    echo '<button class="btn btn-success" type="button" id="add-customer-button" data-bs-toggle="modal" data-bs-target="#addAccount"><i class="bi bi-plus-circle"></i>Add</button>'; // Thêm nút "Thêm" với icon plus-circle
    echo '</div>';
    echo '</div>';
    ...
    if ($dataRole) {
        foreach ($dataRole as $role) {
            ...
            echo '<button class="btn btn-success me-1" data-role-id="' . $role['role_id'] . '" data-bs-toggle="modal" data-bs-target="#updateRoleModel" onclick="openUpdateModel(' . $role['role_id'] . ')"><i class="bi bi-pencil-square"></i>Update</button>';
            echo '<button class="btn btn-danger" onclick="deleteAccount(\'' . $role['role_id'] . '\')"><i class="bi bi-archive"></i>Delete</button>';
            echo '</td>';
            echo '</tr>';
            $stt++;
        }
